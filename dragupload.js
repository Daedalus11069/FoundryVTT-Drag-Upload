...
/**
 * Safely delegate a drop event to Foundry's canvas handler when available.
 * If the internal API is missing (or the scene/canvas isn't ready) call a
 * public Hook fallback so other modules can respond instead of throwing.
 */
function safeCanvasDrop(event) {
  try {
    if (
      typeof canvas !== "undefined" &&
      canvas &&
      typeof canvas._onDrop === "function"
    ) {
      return canvas._onDrop(event);
    }
  } catch (err) {
    // swallow errors from calling private APIs
    console.warn("DragUpload | canvas._onDrop threw:", err);
  }
  ...
  const { x, y } = canvas.canvasCoordinatesFromClient({
    x: event.clientX,
    y: event.clientY
  });
  data.x = x;
...
async function handleDrop(event) {
  ...
  if (!files || files.length === 0) {
    ...
    if (!url) {
      ...
      // Let Foundry handle the event instead (safe)
      return safeCanvasDrop(event);
    }
...
async function CreateActor(event, file) {
  ...
  if (
    Object.keys(CONST.IMAGE_FILE_EXTENSIONS).filter(x => file.name.endsWith(x))
      .length == 0
  ) {
    data.img = "";
  }
